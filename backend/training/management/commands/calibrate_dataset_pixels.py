...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if calibration_file.exists():
            try:
                ...
                for record in existing_data.get('calibration_records', []):
                    try:
                        existing_records[int(record['id'])] = record
                    except (KeyError, TypeError, ValueError):
                        continue
                if skip_existing:
                    self.stdout.write(f'📚 Cargados {len(existing_records)} registros de calibración existentes')
            except Exception as exc:
                self.stdout.write(self.style.WARNING(f'⚠️ No se pudo cargar calibración previa: {exc}'))

        if skip_existing and not existing_records:
            self.stdout.write(self.style.WARNING('⚠️ --skip-existing solicitado pero no se encontró JSON válido. Se reprocesará todo.'))
            skip_existing = False
        ...
        for _, row in valid_df.iterrows():
            ...
            try:
                if skip_existing and image_id in existing_records:
                    calibration_data.append(existing_records[image_id])
                    skipped_count += 1
                ...
                try:
                    ...
                    self.stdout.write(self.style.WARNING(f'  [WARN] Segmentación básica falló para {image_filename}: {seg_exc}'))
                    ...
                    if not crop_result.get('success'):
                        raise SegmentationError(crop_result.get('error', 'Segmentación fallback falló'))
                    crop_path = Path(crop_result['crop_path'])
                ...
                ensure_dir_exists(processed_png_path.parent)
                if crop_path.resolve() != processed_png_path.resolve():
                    shutil.copy2(crop_path, processed_png_path)

                with Image.open(processed_png_path) as crop_image:
                    crop_array = np.array(crop_image)
                    crop_width, crop_height = crop_image.size

                if crop_array.ndim == 3 and crop_array.shape[2] == 4:
                    alpha = crop_array[:, :, 3]
                    mask = (alpha > 128).astype(np.uint8)
                else:
                    mask = np.ones(crop_array.shape[:2], dtype=np.uint8)
                ...
                if len(x_coords) > 0:
                    ...
                    width_pixels = crop_width
                    height_pixels = crop_height
                ...
                scale_factor_alto = alto_real / height_pixels if height_pixels > 0 else 0.0
                scale_factor_ancho = ancho_real / width_pixels if width_pixels > 0 else 0.0
                scale_factor_promedio = (scale_factor_alto + scale_factor_ancho) / 2 if (height_pixels > 0 and width_pixels > 0) else 0.0

                background_pixels = original_pixels_total - grain_area_pixels
                background_ratio = background_pixels / original_pixels_total if original_pixels_total > 0 else 0.0
                ...
                calibration_record = {
                    ...
                    'pixel_measurements': {
                        ...
                        'aspect_ratio': width_pixels / height_pixels if height_pixels > 0 else 0.0
                    },
                    'background_info': {
                ...
                existing_records[image_id] = calibration_record
                processed_count += 1
                ...
                self.stdout.write(self.style.ERROR(f'  ❌ Error de segmentación para {image_filename}: {seg_error}'))
                error_count += 1
            except Exception as exc:
                self.stdout.write(self.style.ERROR(f'  ❌ Error procesando {image_filename}: {exc}'))
                error_count += 1
